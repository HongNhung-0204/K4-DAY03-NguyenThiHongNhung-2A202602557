...
> **Mã Sinh Viên / Mã Học viên:** 2A202602557
...
| **2. Tool Interaction**     |     2 / 5      | Vị trí kệ, trạng thái mượn/trả và hạn trả là dữ liệu thời gian thực trong catalog thư viện. Agent bắt buộc gọi MCP Server (1 tool tra cứu + 1 tool hành động gia hạn). Không được bịa số kệ hay hạn trả. Chatbot cấp 2 không truy cập được CSDL nên không giải được bài toán.                                                    |
...
| **TỔNG ĐIỂM AGENTIC FIT**   |  **15 / 20**   | _Tổng > 12/20: bài toán phù hợp triển khai ReAct Agent + MCP. Chatbot baseline chỉ trả lời quy định thư viện chung; phần tra cứu vị trí/tình trạng và gia hạn phải đi qua Agent._                                                                                                                                                |

---

## 2. TRÍCH XUẤT KẾT QUẢ WATERFALL TRACE LOG

> ⚠️ **Trạng thái hiện tại:** Đã nhận API key và kích hoạt `GeminiProvider`. Suite chạy đủ 5/5 test case; các lượt đầu nhận phản hồi từ API gemini 3.6

Dưới đây là đoạn trích từ file `docs/trace_waterfall.json` sau lần chạy mixed live/fallback:
...
[
  {
    ...
    "thought": "Gemini quyết định gọi công cụ 'library_query' với tham số: {\"book_id\": \"BK2026001\"}",
    "query": "Hãy tra cứu vị trí kệ và tình trạng mượn trả của cuốn sách BK2026001.",
  ...
  },
  {
    "step": 2,
    "action_type": "TOOL_EXECUTION",
    "thought": "Gemini quyết định gọi công cụ 'renew_loan' với tham số: {\"reader_id\": \"RD2026001\", \"extra_days\": 7, \"book_id\": \"BK2026001\"}",
...
- [x] Đã xác nhận toàn bộ Agent chạy bằng LLM API thật (lần chạy này bị giới hạn quota và có fallback).
